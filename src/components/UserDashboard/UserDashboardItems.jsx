  ...
  useEffect(()=>{
    const checkUserOrderStatus = async()=> {
      ...
      if(response.status===401){
        setDisplayMessage(`No menu added for ${formatDateToDateString(response.date)} yet, please check back after 10 minutes`);
        ...
        if(response.data.user_order.length === 0 ){
          ...
          setOrdersCloseAtMessage(`Note; orders for this menu close at ${formatDateToDateAndTimeString(response.data.expires_at)}`)
          setOrdered(false)
        }else {
          // nigga has ordered.
          setDisplayMessage(
            `You have ordered ${response.data.user_order[0].food_name} ${response.data.user_order[0].drink_name &&  `and  ${response.data.user_order[0].drink_name}` } for ${ (response.data.menu_date.split('T')[0] === today.toISOString().split('T')[0]) ? 'today' : ''} ${formatDateToDateString(response.data.menu_date)}`
            )
