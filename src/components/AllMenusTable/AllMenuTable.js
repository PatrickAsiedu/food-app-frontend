...
const editMenuItem =(menu) =>{
  ...
  return (
    ...
    (<div className="hidden sm:flex sm:flex-col w-full  box-outer-shadow mt-12 rounded-3xl px-9 ">
      ...
      {menus.map(menu=>(
        <div key={menu.menu_id} className="w-full  bg-tablehighligh/50 grid grid-cols-4 text-sm gap-3 mb-6">
        <div className="py-4 font-medium break-words pl-3">
          {menu.foods.map(food=>(
            <h1 key={food.food_id} className="mt font-medium break-words mb-2">* {food.food_name}</h1>
          ))}
        </div>
        <div className="py-4 font-medium break-words pl-3">
        {menu.drinks.map(drink=>(
            <h1 key={drink.drink_id} className="mt font-medium break-words mb-2">* {drink.drink_name}</h1>
          ))}
        </div>
        <h1 className="py-4 font-medium break-words">
          {formatDateToDateString(menu.menu_date)}
        </h1>
        ...
        {canEdit(menu.menu_date) && (
          ...
          <button className="ml-12" onClick={()=>deleteMenuItem(menu.menu_id)}>
            <svg
